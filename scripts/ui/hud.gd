...
func _update_restart_button_visibility() -> void:
	# Only the host/server may restart. Offline (no peer) counts as local authority.
...
func _bind_local_player() -> void:
	...
	for p in players:
		if p is CharacterBody2D and int(p.get("peer_id")) == want_id:
			_track_player(p as CharacterBody2D)
			return
	# Not ready yet — keep trying from _process; do NOT fall back to another peer.
...
func _disconnect_tracked_signals() -> void:
	...
	if inv:
		...
		if inv.shield_changed.is_connected(_on_shield_signal):
...
func _pull_player_state() -> void:
	## Always refresh from the tracked player's current property values.
	## Covers RPCs that mutate state without a dedicated HUD signal path.
	if _tracked_player == null or not is_instance_valid(_tracked_player):
		return
	var h: HealthComponent = _tracked_player.get_node_or_null("Health") as HealthComponent
	var inv: InventoryComponent = _tracked_player.get_node_or_null("Inventory") as InventoryComponent
...
func _process(_delta: float) -> void:
	_update_restart_button_visibility()
	var want_id: int = _local_peer_id()
	if _tracked_player == null or not is_instance_valid(_tracked_player) or _bound_peer_id != want_id:
		_bind_local_player()
		return
	# Keep power-up text live while speed ticks down.
	var h: HealthComponent = _tracked_player.get_node_or_null("Health") as HealthComponent
	var inv: InventoryComponent = _tracked_player.get_node_or_null("Inventory") as InventoryComponent
...
func _refresh_match() -> void:
	...
	# Match state changes often accompany player state (tag score, restart) — refresh HUD.
	_pull_player_state()
...
func _on_restart() -> void:
	# Client UI should be hidden; still ignore if a non-server somehow fires this.
